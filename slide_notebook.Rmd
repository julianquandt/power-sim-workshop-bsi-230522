...
## STEP 1: Calculate what number of heads with a chance < alpha of a fair coin
...
```{r fair_coin}
fair_coin_events <- data.frame(replicate(10, 0:1)) # tails = 0 , heads = 1
fair_coin_events
```
...
```{r easy_step2}
pbinom(q = 8, size = 10, prob = .6, lower.tail = FALSE)
```
...
while(check_power < .80){
  ...
  # calculate the proportion of sequences that result in more than crit_heads heads with the unfair coin
  power_at_n[i] <- mean(rbinom(100, n_tosses_simulated, .6) > crit_heads_simulated[i])
  check_power <- power_at_n[i] # save the check power for the loop
  # if power is still < .80
  if(check_power < .80){
...
$happiness_31 = 0 + 2*1 + \epsilon_31$ = $happiness_31 = 0 + 2 + \epsilon_31$
...
for(i in 1:nrow(group_data)){
  ...
  group_data$happiness[i] <- rnorm(1, 
